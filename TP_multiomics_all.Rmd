...
2. Vous travaillez sur un modèle murin et vous souhaitez comprendre les impacts
   ...
   ---------------------------------------------------------
   Dans le cas de cet étude, on a à disposition des données de séquençage de type 16 ainsi que des données de métabolomiques(décompte pour chaque nucléotide). 
   
   Comme pour les purs polaires, on peut commencer par une PCA afin de voir la distribution des données métabolomique selon la PCA. 
   Ensuite, on peut s'appuyer sur une PLS.Elle s'applique sur les  deux ensembles de données étabolomiques et les especes bactériennes issues des données de séquençage. Ceci va permettre d'extraire des informations sur les données  corrélées et mettre en évidence les points communs entre les ensembles de données.
