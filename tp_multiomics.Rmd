...
Cette méthode est beaucoup utilitée pour des données multivariées. Ce pendant, les données doivent etre normalisées pour éviter que le résultat ne soit fausser par le poids d'une variables tres abondante par rapport aux autres variables.
...
```{r}
mRNA_pca = PCA(mRNA_filtre, scale.unit = TRUE)
get_eigenvalue(mRNA_pca)
#mRNA_pca$eig
```
...
```{r}
# pour 50%  ----> dimension = 6
mRNA_pca = PCA(mRNA_filtre, scale.unit = TRUE, ncp = 6)
get_eigenvalue(mRNA_pca)
...
var_dim1
data()
print(spca(as.matrix(mRNA_filtre),  ncomp = 1))
```
