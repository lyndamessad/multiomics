...
```{r}
plotIndiv(pls_rna_prot,group =  sampl_group$Y, ellipse = TRUE, col = c("red", "blue", "darkgreen"), style = "ggplot2", comp = c(2,3),legend = TRUE, title = 'Individu Scatter plot colored by group, composante 2 and 3')
```
...
plotArrow(pls_rna_prot, comp = c(1,3),
          ...
          X.label = "PLS comp1",
          Y.label = "PLS comp3",
          title = "Arrow plot of mRNA and protein PLS"
          )
```
...
```{r}
network(RNA_prot.spls, block.var.names = name, cutoff =0.63, save = 'jpeg', name.save = 'sPLSnetwork') #tout le network +++ a affihcer sur la console 
# se sauvegarde automatique dans le repertiore courant
