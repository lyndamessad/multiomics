...
```{r}
miRNA_filtre = miRNA[,names(which(abs(apply(miRNA, 2, coef_var)) >= 0.15))] ; class(miRNA_filtre)
...
```
  -----> exemple des variables les plus corrélées :  protéine "ER.alpha" avec les genes "ZNF552"(coef = 0.73) et "KDM4B" (coef = 0.77) puis la potéine "GATA3" avec le gene "KDM4B" (coeff  = 0.71). Il s'agit de corrélations positives.
...
pls_rna_prot_mirna =block.pls(X =list(mRNA,  prot_filtre) ,Y =miRNA_filtre ,ncomp = 2, scale = T)
pls_rna_prot_mirna
...
```{r}
RNA_prot_miRNA.spls =block.spls(X= list(mRNA_filtre, prot_filtre), Y = miRNA, ncomp = 1, 
