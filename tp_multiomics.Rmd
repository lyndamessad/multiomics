...
```{r}
plotIndiv(pls_rna_prot,group =  sampl_group$Y, ellipse = TRUE, col = c("red", "blue", "darkgreen"), style = "ggplot2", comp = c(2,3),legend = TRUE, title = 'Individu Scatter plot colored by group, composante 2 and 3', size.title = rel(1.5))
```
...
```{r}
most_corr_positive = list()
most_corr_negative = list()

for (i in range(length(correlation))){
   if (correlation[i] >= 0.7){
      most_corr_positive <- c(most_corr_positive, correlation[i])
   }
   if (correlation[i] <= -0.7){
      most_corr_negative <- c(most_corr_negative, correlation[i])
   }
}
most_corr_positive
correlation
```
   -----> Les plus corrélés sont :  protéine "ER.alpha" avec les genes "ZNF552"(coef = 0.73) et "KDM4B" (coef = 0.77) puis la potéine "GATA3" avec le gene "KDM4B" (coeff  = 0.71). Il s'agit de corrélations positives 
...
```{r}
pls_rna_prot_mirna =pls(X =as.matrix(mRNA_filtre, prot_filtre) ,Y =miRNA_filtre ,ncomp = 2, scale = T)
pls_rna_prot_mirna
...
```{r}
RNA_prot_miRNA.spls =spls(X= as.matrix(mRNA_filtre, prot_filtre), Y = miRNA, ncomp = 1, 
